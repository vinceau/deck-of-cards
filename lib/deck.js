...
import showHand from './modules/showhand'
...
export default function Deck (sixplayer) {
  ...
  var self = observable({mount, unmount, cards, $el, transform, addKitty})
  var $root
  ...
  function transform (values) {
    if (values.length !== self.hand.length) return
    values.forEach(function (newi, i) {
      var c = self.hand[i]
      c.morph(newi)
    })
  }

  function addKitty (newHand) {
    var new_hand = new Array(newHand.length)
    var kittens_found = 0

    newHand.forEach(function(newi, i) {
      var found = false
      ...
      if (!found) {
        var kit = self.kitty[kittens_found++]
        kit.morph(newi)
        kit.setSide('front')
        new_hand[i] = kit
      }
...
Deck.ease = ease
Deck.modules = {bysuit, fan, intro, deal, shuffle, sort, flip, showHand}
Deck.Card = Card
Deck.prefix = prefix
